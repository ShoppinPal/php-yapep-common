 ...
 *
 * @todo turn echologger and stderrlogger into a streamlogger
 */
class EchoLogger extends LoggerAbstract
{
    /**
    ...
    protected function logMessage(IMessage $message)
    {
        if ($message->getPriority() <= $this->minimumLogLevel) {
            echo sprintf(
                "%s %s(%d) [%s]: %s %s\n",
                date('d-M-Y H:i:s'),
                str_pad($this->logLevelHelper->getTextFromLevel($message->getPriority()), 6),
                $message->getPriority(),
                str_pad($message->getTag(), 20),
                $message->getMessage(),
                (count($message->getFields()) > 0 ? sprintf('(DATA: "%s")', json_encode($message->getFields())) : '')
            );
        }
    }
}
